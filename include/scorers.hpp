  ...
  std::vector<int32_t> score(const std::vector<Exam> &exams,
    ...
    for (size_t i = 0; i < exams.size(); ++i) {
      ...
      for (; j + BATCH_SIZE < correct_answers.size(); j += BATCH_SIZE) {
        ...
        const __m256i vec_answers = _mm256_set_epi8(
        ...
        const __m256i vec_points = _mm256_set_epi8(
        ...
        const __m256i vec_final_points =
            _mm256_and_si256(vec_marks, vec_points);
        // Reference:
        ...
        const __m256i vec_final_points_4_sum =
            _mm256_sad_epu8(vec_final_points, _mm256_setzero_si256());
